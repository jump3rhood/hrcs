...
export default function CandidateProfilePage() {
  ...
  const [profile, setProfile] = useState<CandidateProfileWithSkills | null>(
    null,
  )
    ...
    setProfile((prev) =>
      prev
        ? {
            ...prev,
            skills: [
              ...prev.skills.filter((s) => s._id !== res.data._id),
              res.data,
            ],
          }
        : prev,
  ...
  const onDeleteSkill = async (skill: CandidateSkill) => {
    ...
    setProfile((prev) =>
      prev
        ? { ...prev, skills: prev.skills.filter((s) => s._id !== skill._id) }
        : prev,
    )
  }
              ...
              >
                ...
                <div className='flex'>
                  {[1, 2, 3, 4, 5].map((n) => (
                    <Star
                      key={n}
                      className={`h-3 w-3 ${n <= skill.rating ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground'}`}
                    />
                  ))}
                </div>
                <button
              ...
              <div className='flex gap-1 mt-1'>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button key={n} type='button' onClick={() => setNewRating(n)}>
                    <Star
                      className={`h-6 w-6 ${n <= newRating ? 'fill-yellow-400 text-yellow-400' : 'text-muted-foreground'}`}
                    />
                  </button>
                ))}
              </div>
            </div>
            <Button onClick={onAddSkill} type='button'>
              Add
            </Button>
          </div>
          <div className='flex gap-1 mt-1'>
            {['React', 'Node.js', 'TypeScript', 'Python', 'MongoDB', 'AWS'].map(
              (s) => (
                <Badge
                  key={s}
                  variant='outline'
